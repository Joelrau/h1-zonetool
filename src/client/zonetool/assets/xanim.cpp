...
namespace zonetool
{
	...
	XAnimParts* IXAnimParts::parse(const std::string& name, ZoneMemory* mem)
	{
		...
		if (asset->blendShapeWeightNames)
		{
			...
			{
				this->add_script_string(&asset->blendShapeWeightNames[name], reader.read_string());
			}
		}
	...
	void IXAnimParts::prepare(ZoneBuffer* buf, ZoneMemory* mem)
	{
		...
		if (xanim->names)
		{
			...
			{
				auto bone = buf->write_scriptstring(this->get_script_string(&xanim->names[i]));
				xanim->names[i] = static_cast<scr_string_t>(bone);
			}
		}
		...
		{
			auto name = buf->write_scriptstring(this->get_script_string(&xanim->notify[i].name));
			xanim->notify[i].name = static_cast<scr_string_t>(name);
		}
		...
		/*if (xanim->blendShapeWeightNames)
		{
			...
			{
				auto name = buf->write_scriptstring(this->get_script_string(&xanim->blendShapeWeightNames[i]));
				xanim->blendShapeWeightNames[i] = static_cast<scr_string_t>(name);
			}
		}*/
	}
