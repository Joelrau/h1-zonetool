...
#define REINTERPRET_CAST_SAFE_ARR(name, count) \
		...
		h2::GfxWorld* convert_to_h2(GfxWorld* h1_asset, utils::memory::allocator& allocator)
		{
			const auto asset = allocator.allocate<h2::GfxWorld>();
			...
			asset->portalGroup = allocator.allocate_array<h2::GfxPortalGroup>(asset->portalGroupCount);
			...
			// dpvs
			{
				...
				asset->dpvs.unkCount2 = h1_asset->dpvs.smodelCount;
				...
				asset->dpvs.smodelDrawInsts = allocator.allocate_array<h2::GfxStaticModelDrawInst>(asset->dpvs.smodelCount);
	...
	void IGfxWorld::dump(GfxWorld* h1_asset)
	{
