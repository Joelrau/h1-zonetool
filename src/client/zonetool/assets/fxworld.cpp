...
namespace zonetool
{
	namespace
	{
		h2::FxWorld* convert_to_h2(FxWorld* h1_asset, utils::memory::allocator& allocator)
		{
			...
			{
				std::memcpy(&asset->glassSys.piecePlaces[i].__s0.frame.quat, &h1_asset->glassSys.piecePlaces[i].__s0.frame.quat,
					sizeof(asset->glassSys.piecePlaces[i].__s0.frame.quat));
				std::memcpy(&asset->glassSys.piecePlaces[i].__s0.frame.origin, &h1_asset->glassSys.piecePlaces[i].__s0.frame.origin,
					sizeof(asset->glassSys.piecePlaces[i].__s0.frame.origin));

				asset->glassSys.piecePlaces[i].__s0.radius = h1_asset->glassSys.piecePlaces[i].__s0.radius;
			}

			asset->glassSys.initPieceStates = allocator.allocate_array<h2::FxGlassInitPieceState>(asset->glassSys.initPieceCount);
			for (auto i = 0u; i < asset->glassSys.initPieceCount; i++)
			{
				std::memcpy(&asset->glassSys.initPieceStates[i].frame.quat, &h1_asset->glassSys.initPieceStates[i].frame.quat,
					sizeof(asset->glassSys.initPieceStates[i].frame.quat));

				std::memcpy(&asset->glassSys.initPieceStates[i].frame.origin, &h1_asset->glassSys.initPieceStates[i].frame.origin,
					sizeof(asset->glassSys.initPieceStates[i].frame.origin));

				asset->glassSys.initPieceStates[i].radius = h1_asset->glassSys.initPieceStates[i].radius;
				asset->glassSys.initPieceStates[i].texCoordOrigin[0] = h1_asset->glassSys.initPieceStates[i].texCoordOrigin[0];
				asset->glassSys.initPieceStates[i].texCoordOrigin[1] = h1_asset->glassSys.initPieceStates[i].texCoordOrigin[1];
				asset->glassSys.initPieceStates[i].supportMask = h1_asset->glassSys.initPieceStates[i].supportMask;
				asset->glassSys.initPieceStates[i].areaX2 = h1_asset->glassSys.initPieceStates[i].areaX2;
				asset->glassSys.initPieceStates[i].lightingIndex = h1_asset->glassSys.initPieceStates[i].lightingIndex;
				asset->glassSys.initPieceStates[i].defIndex = h1_asset->glassSys.initPieceStates[i].defIndex;
				asset->glassSys.initPieceStates[i].vertCount = h1_asset->glassSys.initPieceStates[i].vertCount;
				asset->glassSys.initPieceStates[i].fanDataCount = h1_asset->glassSys.initPieceStates[i].fanDataCount;
				*asset->glassSys.initPieceStates[i].pad = *h1_asset->glassSys.initPieceStates[i].pad;
			}
	...
	static_assert(sizeof(FxWorld) == sizeof(h2::FxWorld));

	void IFxWorld::dump(FxWorld* h1_asset)
	{
