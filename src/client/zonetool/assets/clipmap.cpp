...
namespace zonetool
{
	...
	void IClipMap::parse_info(ClipInfo* info, assetmanager::reader& read, ZoneMemory* mem)
	{
		...
		info->bCollisionTree.leafbrushNodes = read.read_array<cLeafBrushNode_s>();
		for (unsigned int i = 0; i < info->bCollisionTree.leafbrushNodesCount; i++)
		{
			if (info->bCollisionTree.leafbrushNodes[i].leafBrushCount > 0)
			{
				info->bCollisionTree.leafbrushNodes[i].data.leaf.brushes = read.read_array<unsigned short>();
			}
		}
		info->bCollisionTree.leafbrushes = read.read_array<unsigned short>();

		info->pCollisionTree.aabbTrees = read.read_array<CollisionAabbTree>();

		info->sCollisionTree.smodelNodes = read.read_array<SModelAabbNode>();

		info->bCollisionData.brushSides = read.read_array<cbrushside_t>();
		info->bCollisionData.brushEdges = read.read_array<unsigned char>();
		info->bCollisionData.brushes = read.read_array<cbrush_t>();
		for (unsigned int i = 0; i < info->bCollisionData.numBrushes; i++)
		{
			if (info->bCollisionData.brushes[i].sides)
			{
				info->bCollisionData.brushes[i].sides = read.read_array<cbrushside_t>();
			}
			if (info->bCollisionData.brushes[i].baseAdjacentSide)
			{
				info->bCollisionData.brushes[i].baseAdjacentSide = read.read_array<cbrushedge_t>();
			}
		}
		info->bCollisionData.brushBounds = read.read_array<Bounds>();
		info->bCollisionData.brushContents = read.read_array<int>();

		info->pCollisionData.verts = read.read_array<vec3_t>();
		info->pCollisionData.triIndices = read.read_array<unsigned short>();
		info->pCollisionData.triEdgeIsWalkable = read.read_array<unsigned char>();
		info->pCollisionData.borders = read.read_array<CollisionBorder>();
		info->pCollisionData.partitions = read.read_array<CollisionPartition>();
		for (int i = 0; i < info->pCollisionData.partitionCount; i++)
		{
			if (info->pCollisionData.partitions[i].borders)
			{
				info->pCollisionData.partitions[i].borders = read.read_array<CollisionBorder>();
			}
		}

		info->sCollisionData.staticModelList = read.read_array<cStaticModel_s>();
		for (unsigned int i = 0; i < info->sCollisionData.numStaticModels; i++)
		{
			if (info->sCollisionData.staticModelList[i].xmodel)
			{
				info->sCollisionData.staticModelList[i].xmodel = read.read_asset<XModel>();
			}
		}
	}
	...
	clipMap_t* IClipMap::parse(const std::string& name, ZoneMemory* mem)
	{
		...
		if (asset->scriptableMapEnts.instances)
		{
			for (unsigned int i = 0; i < asset->scriptableMapEnts.instanceCount; i++)
			{
				...
				{
					asset->scriptableMapEnts.instances[i].targetData = read.read_array<ScriptableInstanceTargetData>();
				}
		...
		asset->grappleData.sphereTreeData.sphereTree = read.read_array<sphere_tree_t>();
		if (asset->grappleData.sphereTreeData.sphereTree)
		{
			for (int i = 0; i < asset->grappleData.sphereTreeData.sphereTreeCount; i++)
			{
				asset->grappleData.sphereTreeData.sphereTree[i].unk = read.read_array<unsigned int>();
			}
		}

		asset->grappleData.sphereTreeData.sphereTreeObj = read.read_array<sphere_tree_obj_t>();

		asset->grappleData.magnet = read.read_array<grapple_magnet_t>();

		read.close();

		// parse stages
		assetmanager::reader stageReader(mem);
		if (stageReader.open(name + ".ents.stages"))
		{
			ZONETOOL_INFO("Parsing entity stages...");

			asset->stageCount = static_cast<unsigned char>(stageReader.read_int());
			if (asset->stageCount)
			{
				asset->stages = stageReader.read_array<Stage>();

				for (int i = 0; i < asset->stageCount; i++)
				{
					asset->stages[i].name = stageReader.read_string();
				}
			}
		}
		stageReader.close();
	...
	void IClipMap::load_depending(IZone* zone)
	{
		...
		if (data->info.sCollisionData.staticModelList)
		{
			for (unsigned int i = 0; i < data->info.sCollisionData.numStaticModels; i++)
			{
				...
				{
					zone->add_asset_of_type(ASSET_TYPE_XMODEL, data->info.sCollisionData.staticModelList[i].xmodel->name);
				}
			}
		}
		...
		for (int i = 0; i < 2; i++)
		{
			if (data->dynEntDefList[i])
			{
				for (int j = 0; j < data->dynEntCount[i]; j++)
				{
					...
					{
						zone->add_asset_of_type(ASSET_TYPE_SOUND, data->dynEntDefList[i][j].sound->name);
					}
				...
				for (unsigned int i = 0; i < data->sCollisionData.numStaticModels; i++)
				{
					if (data->sCollisionData.staticModelList[i].xmodel)
					{
						staticModelList[i].xmodel = reinterpret_cast<XModel*>(zone->get_asset_pointer(
							ASSET_TYPE_XMODEL, data->sCollisionData.staticModelList[i].xmodel->name));
					}
				}
			}
		}
			...
			for (std::uint16_t i = 0; i < data->dynEntCount[0]; i++)
			{
				...
				if (data->dynEntDefList[0][i].sound)
				{
					dyn_entity_def[i].sound = reinterpret_cast<snd_alias_list_t*>(zone->get_asset_pointer(
						ASSET_TYPE_SOUND, data->dynEntDefList[0][i].sound->name));
				}
